...
class League(object):
    ...
    def __init__(self, calendar, teams_names, n_days, SE=0):
        ...
        self.rank_data = {i:[self.teams[i].get_league_points(),self.teams[i].get_abs_points(),\
                             self.teams[i].get_goals_scored()] for i in teams_names}
...
class Stats(object):
    ...
    def __init__(self, list_leagues):
        ...
        self.teams_frankings = {i: [0 for _ in range(len(self.teams_names))] for i in teams_names}
        self.gen_frankings()
    ...
    def plot_frankings(self):
        for i in range(len(teams_names)):
            plt.plot(self.get_frankings_team(teams_names[i]))
